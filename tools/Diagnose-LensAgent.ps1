...
function Get-BeatAge {
    if (Test-Path -LiteralPath $beat) {
        return [int]((Get-Date) - (Get-Item -LiteralPath $beat).LastWriteTime).TotalSeconds
    }
    return $null
}
...
function Show-Snapshot {
    ...
    if (Test-Path -LiteralPath $stop) {
        $reason = (Get-Content -LiteralPath $stop `
                               -Raw `
                               -ErrorAction SilentlyContinue) -replace '\s+$', ''
        Write-Host ("  PRESENT -> '{0}'  (agent was told to exit)" -f $reason) -ForegroundColor Red
    ...
    if (Test-Path -LiteralPath $strikes) {
        $count = (Get-Content -LiteralPath $strikes `
                              -Raw `
                              -ErrorAction SilentlyContinue) -replace '\s+$', ''
    ...
    if ($procs) {
        $procs | ForEach-Object { Write-Host "  PID $($_.ProcessId) is running LensAgent.ps1" -ForegroundColor Green }
...
if ($info) {
    $info | Select-Object LastRunTime, LastTaskResult, NumberOfMissedRuns | Format-List | Out-String | Write-Host
...
if (Test-Path -LiteralPath $log) {
    $hits = Get-Content -LiteralPath $log `
                        -Tail 500 `
                        -ErrorAction SilentlyContinue |
...
Write-Host '  heartbeat past ~4s / stop.flag / no agent process -> agent dead or wedged (next pick recycles it).'
Write-Host '  request-*.bin lingers while the beat stays fresh  -> serve loop not reading it (wrong dir).'
Write-Host '  request consumed, no result-*.bin within ~60s     -> a lookup job failing or hanging (AD / SCCM).'
