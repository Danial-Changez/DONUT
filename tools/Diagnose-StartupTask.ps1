...
function Write-StartupTaskLog {
    ...
    if ($lines.Count -eq 0) {
        Write-Host ("Donut.log has no 'Startup task' lines - Apply() has never run. The toggle is off, or its " +
            "side effect never fired.") -ForegroundColor Yellow
        return
    }
...
function Test-PathReachable([string]$path) {
    ...
    $leafExists = Test-Path -LiteralPath $leaf
    Write-Host ("  exists to THIS shell: " + $(if ($leafExists) { 'yes' } else { 'NO' })) `
               -ForegroundColor $(if ($leafExists) { 'Green' } else { 'Red' })
    ...
    while ($dir -and (Split-Path $dir -Parent)) {
        try {
            ...
            $tone = if ($users.Count) { 'Gray' } else { 'Red' }
            Write-Host ("  {0,-55} {1}" -f $dir, $verdict) -ForegroundColor $tone
...
function Get-ResultMeaning([uint32]$code) {
    ...
    $map = @{
        ...
        '0x800702E4' = 'ELEVATION REQUIRED - CreateProcess refused an elevation-requiring exe ' +
        'under a non-elevated token'
        '0x800704DD' = 'NOT LOGGED ON - the principal has no interactive session'
        '0x800704EC' = 'BLOCKED BY POLICY - AppLocker/WDAC/SRP refused this exe or script for this token, ' +
        'which is NOT a file ACL'
    ...
    foreach ($log in 'Microsoft-Windows-AppLocker/EXE and DLL',
        ...
        if ($ev.Count -gt 0) {
            ...
            $ev | Select-Object -First 5 | ForEach-Object {
                ...
                Write-Host ("  {0}  id={1}  {2}" -f $_.TimeCreated, $_.Id, $firstLine) -ForegroundColor Red
            }
        } else { Write-Host "$log - no DONUT events" }
    }

    # WDAC/Code Integrity is the other allowlisting engine and blocks the same way.
    $ciFilter = @{ LogName = 'Microsoft-Windows-CodeIntegrity/Operational'; Id = 3076, 3077 }
    $ci = @(Get-WinEvent -FilterHashtable $ciFilter `
                         -MaxEvents 40 `
                         -ErrorAction SilentlyContinue | Where-Object { $_.Message -match 'DONUT' })
    if ($ci.Count -gt 0) {
        $script:policyBlocked = $true
...
if (-not (Test-Path -LiteralPath $svcPath)) {
    Write-Host ("No extracted app tree at $srcRoot - " +
        "DONUT may run from the dev path (Start-Donut.ps1).") -ForegroundColor Yellow
} else {
    ...
    # One verdict line per fix: label, then PRESENT/MISSING in green or red.
    function Write-FixLine([string]$label, [bool]$ok, [string]$yes = 'PRESENT', [string]$no = 'MISSING') {
        $verdict = if ($ok) { $yes } else { $no }
        Write-Host ("$label : $verdict") -ForegroundColor $(if ($ok) { 'Green' } else { 'Red' })
    }
    Write-FixLine 'token-owner fix         ' $hasToken
    Write-FixLine 'console-user trigger fix' $script:codeSplitsTrigger
    Write-FixLine 'de-elevated autostart   ' (-not $script:codeHasOldLane) 'YES' 'NO (old SYSTEM lane)'
    if ($script:codeHasOldLane -or -not $script:codeSplitsTrigger) {
        Write-Host ("The installed launcher predates the current fixes. Rebuild Donut.Launcher.exe and reinstall " +
            "- a git pull does not update an installed build.") -ForegroundColor Red
    }
}
...
if (-not $task) {
    ...
    $expectedLabel = if ($expected) { $expected }
    else { '(no signed-in console user - Apply refuses to register without one)' }
    Write-Host "expected name: $expectedLabel"
    ...
    Write-Host ("Registering a task needs an elevated token. A de-elevated DONUT reaches " +
        "Register-ScheduledTask, gets access denied, and reports it as one toast - which is what an " +
        "absent task plus an access-denied line in Donut.log means.") -ForegroundColor Yellow
    Write-Host ("DO THIS: confirm above whether startWithWindows is true. If it is, relaunch DONUT as " +
        "administrator and toggle it off and on - it should register immediately. If that works, the " +
        "bug is that the toggle is not gated behind the elevation prompt.") -ForegroundColor Green
    return
}
...
Get-CimInstance Win32_Process -Filter "Name='explorer.exe'" -ErrorAction SilentlyContinue |
    ForEach-Object {
        $o = Invoke-CimMethod -InputObject $_ `
                              -MethodName GetOwner `
                              -ErrorAction SilentlyContinue
...
foreach ($t in $task.Triggers) {
    ...
    if ($t.UserId -and $consoleUser -and $t.UserId -ine $consoleUser) {
        ...
        Write-Host ("PROBLEM: the logon trigger is bound to '$($t.UserId)', but '$consoleUser' is who signs in " +
            "at the console. That account never logs on interactively, so this task stays Ready and " +
            "never fires.") -ForegroundColor Red
    }
}
...
if ([int]$task.Settings.Priority -ge 7) {
    Write-Host ("PROBLEM: priority $($task.Settings.Priority) runs the whole boot below normal CPU class " +
        "while every other logon app competes for the disk. Toggle Start with Windows off and on to " +
        "re-register it at 5.") -ForegroundColor Red
}
...
if ($action.Execute -like '*PsExec*' -or $action.Arguments -like '*Start-DonutInConsoleSession*') {
    Write-Host ("PROBLEM: this task is from an older build that started DONUT as SYSTEM via psexec. As " +
        "SYSTEM it authenticates on the network as the machine account, which has no rights on fleet " +
        "targets, so every remote job fails on access denied. Toggle Start with Windows off and on " +
        "to re-register it.") -ForegroundColor Red
}
if ($principal.UserId -match 'SYSTEM') {
    Write-Host ("PROBLEM: the task runs as SYSTEM. Current builds run it as the console user at their own " +
        "level and elevate on demand.") -ForegroundColor Red
}
if ($principal.RunLevel -eq 'Highest') {
    ...
    try {
        ...
        Write-Host "(could not enumerate local Administrators: $($_.Exception.Message))" -ForegroundColor DarkGray
    }
    if ($direct) {
        Write-Host ("principal '$($principal.UserId)' is a direct local admin - RunLevel Highest will yield an " +
            "elevated token.")
    } else {
        Write-Host ("NOTE: '$($principal.UserId)' is not a DIRECT member of local Administrators. It may still " +
            "be admin via a domain group (Domain Admins), which this check cannot see. IF it is not, " +
            "RunLevel Highest degrades to a standard token and the requireAdministrator launcher refuses " +
            "to start (0x800702E4).") -ForegroundColor Yellow
    }
}
...
if ($script:lastResult -eq 267011) {
    Write-Host ("DIAGNOSIS: registered but NEVER RUN. The task itself is fine - its trigger never fired. " +
        "Check the trigger user above against the console user.") -ForegroundColor Red
}
if ($script:lastResult -eq 2147943660) {
    Write-Host ("DIAGNOSIS: BLOCKED BY POLICY (0x800704EC). An allowlisting policy refused the action for " +
        "this token - the file ACL is not involved, which is why the path still reads Full Control. " +
        "Section 6 names the rule.") -ForegroundColor Red
}
if ($action.Execute -like '*PsExec*' -and $script:lastResult -gt 4) {
    Write-Host ("NOTE: this old-shape action calls psexec -d directly, whose exit code is the launched PID - " +
        "a result like this usually means psexec SUCCEEDED. " +
        "The shim-shaped action exits 0 instead.") -ForegroundColor Yellow
}
...
if ($events) {
    $events | Select-Object TimeCreated, Id, @{ n = 'Message'; e = { ($_.Message -split "`n")[0] } } |
...
Write-Host ("console session id: " +
    "$((Get-Process -Name explorer -ErrorAction SilentlyContinue | Select-Object -First 1).SessionId)")
Write-Host ("this shell session: $((Get-Process -Id $PID).SessionId)   running as: " +
    "$([System.Security.Principal.WindowsIdentity]::GetCurrent().Name)")
$donut = Get-Process -Name 'Donut.Launcher' -ErrorAction SilentlyContinue
$running = if ($donut) { "yes (pid $($donut.Id), session $($donut.SessionId))" } else { 'no' }
Write-Host "DONUT running now: $running"
...
foreach ($proc in $running) {
    ...
    $owner = $null
    if ($cim) {
        $owner = Invoke-CimMethod -InputObject $cim `
                                  -MethodName GetOwner `
                                  -ErrorAction SilentlyContinue
    }
    ...
    if ($null -ne $consoleSession -and $proc.SessionId -ne $consoleSession) {
        Write-Host ("  PROBLEM: not in the console session ($consoleSession) - it has no desktop, so no tray " +
            "icon or window can ever appear.") -ForegroundColor Red
    }
    if ($who -like '*\SYSTEM') {
        Write-Host ("  NOTE: as SYSTEM it authenticates on the network as the machine account, which has no " +
            "rights on fleet targets - remote jobs will fail with access denied.") -ForegroundColor Yellow
    }
}
...
if (Test-Path -LiteralPath $dataRoot) {
    ...
    Write-Host ("data root: $dataRoot - not present " +
        "(DONUT has not run since the move off %LOCALAPPDATA%)") -ForegroundColor Yellow
}
...
$systemProfiles = @(
    ...
    "$env:SystemRoot\SysWOW64\config\systemprofile"
)
foreach ($profileRoot in $systemProfiles) {
...
Write-Host ("If that surfaces DONUT here but the logon task does not, the failure is session-0 " +
    "injection, not the command.")
...
if ($script:policyBlocked) {
    Write-Host ("An allowlisting policy blocked DONUT. This is NOT an ACL problem, which is why the " +
        "directory still shows Full Control.") -ForegroundColor Red
    Write-Host ("The launcher executes src\Start-Donut.ps1 from $appRoot. ProgramData is user-writable, so " +
        "AppLocker/WDAC commonly forbid running scripts from it, while the default rule set exempts " +
        "BUILTIN\Administrators - so an elevated DONUT runs and a de-elevated one is refused.") -ForegroundColor Yellow
    Write-Host ("DO THIS: either have the app tree path allowlisted for your account, or move the extraction " +
        "root into the already-approved install directory so the exe and its scripts share one " +
        "allowed location.") -ForegroundColor Green
}
...
if ($oldShape -and $script:codeSplitsTrigger -and -not $script:codeHasOldLane) {
    Write-Host ("The installed code is current, but this task still has an OLD shape (a SYSTEM principal, a " +
        "psexec action, and/or a trigger bound to a non-console account).") -ForegroundColor Yellow
    Write-Host ("The task is a snapshot from whichever build last applied it - the current code has not " +
        "re-registered yet.") -ForegroundColor Yellow
    Write-Host ("DO THIS: launch DONUT and wait ~2 minutes (the startup-task heal re-applies on a timer), or " +
        "toggle Start with Windows off and on. It will register DONUT-<console user> to run as that " +
        "user, and sweep this stale task. Then re-run this script.") -ForegroundColor Green
} elseif ($oldShape) {
    Write-Host "This task has the OLD shape AND the installed build lacks the fix." -ForegroundColor Red
    Write-Host ("DO THIS: rebuild Donut.Launcher.exe from the current source and reinstall - an installed " +
        "build runs src\ from INSIDE the exe, so pulling alone changes nothing. Then launch DONUT " +
        "and re-run this script.") -ForegroundColor Green
} elseif (-not $script:policyBlocked) {
    Write-Host ("Task shape looks correct: the console user is both the trigger and the principal, at " +
        "RunLevel Highest.") -ForegroundColor Green
    Write-Host ("If DONUT is running but invisible, section 5 says why: a session other than the console one " +
        "means it never reached your desktop; the console session means the process is there but its " +
        "UI is not.") -ForegroundColor Green
}
