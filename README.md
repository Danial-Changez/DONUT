...
- **User Lens:** Picking a person in the finder opens a Lens in the detail pane — their directory info (UPN, email, manager, office) and their SCCM devices (model + last-sync), with BitLocker recovery keys revealed on click and each device addable to the machine list. The SCCM/BitLocker lookup runs **de-elevated as your logged-on account** (DONUT itself runs elevated as an admin account, which can't read them).
