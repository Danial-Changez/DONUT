...
the rest reuse it (~30 ms). Priming does not make concurrency safe — the deadlock is
on the lock, not a cache miss — so serialization is mandatory, not an optimization.
...
user is waiting on, and no UI-thread work inside a poll loop — it gets charged to
whichever job is read next.
...
- `Shell.Application` de-elevation: only de-elevates within the same user — rejected
  for the agent launch (scheduled task with `LogonType Interactive` instead).
...
presenters — bindings are the default render path. A rename to
...
prompt, putting the main window and the login modal on screen together — the app
