...
| `ActiveDirectoryService` | Live multi-forest AD search (computers + users; the search runs inside the de-elevated Lens agent) and account unlock (on the pool) |
...
| `FinderPresenter` | The search bar's live multi-forest AD finder (debounced search over the de-elevated Lens agent + inline unlock) and the **user Lens** (agent lookup, partial streaming, in-memory TTL cache); raw pool jobs polled on `DispatcherTimer`s. Calls back into `HomePresenter`'s machine seams (`PrefetchIp`, `EnsureRow`, `StartInventory`, `MoveRowToTop`, `UpdateEmptyHint`) via the duck-typed back-ref |
