...
| Startup errors (`ErrorDialog`) | Modal Windows | A hand-themed WinForms window, since these fire before WPF and two of them fire *because* it failed. Popup chrome contract, and **no footer at all**: an error carries no decision, and a dismiss-only button is banned. One-line reason, a short action line in the docs' voice, then the exception behind a Details toggle on `TerminalBackground`, read-only so it can be pasted. Every call site passes reason and detail separately. |
| Elevation state | Status Feedback | A standing amber `LIMITED` badge (`BadgeAmber` recipe) beside the wordmark, full sentence in the tooltip. Driven by `ElevationContext::IsElevated()`, never by `runAsAdmin` — the badge reports what the process actually got. The running version sits beside it in the same tier as a `ButtonBadge`, copying on click, because reporting one is the only reason it is on screen. |
