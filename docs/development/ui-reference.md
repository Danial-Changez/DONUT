...
|---|---|---|
| **Title Case** | Buttons, links that act, menu items, headings, pane and section titles, tab and segment names, setting labels, dialog titles, toast titles, status chips | `Clear Selected`, `Run All`, `Reveal Key`, `Close to Tray` |
| **Sentence case** | Tooltips, hint and helper text, blank-slate copy, dialog message bodies, toast bodies | `Clears the folder contents and keeps the folder.` |
...
| Offline state | Status Feedback | One carrier: the red dot plus the `Offline` chip on the machine card. The detail title and the card subtitle no longer repeat the word (they did, three times on one screen), so the chip is where offline reads and the hostname stays a clean copy target. |
| Stacked text in cards | Cards | Montserrat sits taller than Segoe UI and the ladder rounded every size up, so the old gaps read cramped. Label to value 8px on stat tiles (`StatValue`), 4px on dense tiles, 3px under a card title, card padding `16,16` (dense tiles `12,10`), machine rows `14,12,8,12`. A stat tile keeps equal ink gaps, measured on a render: 16 between label, value and sub line, 18 from the top and bottom edges (`Padding="16,14,16,12"`, `StatValue` margin 10, sub line margin 8, or 6 on a copy pill, whose 2px chrome makes up the difference). The sub line starts flush with the value, not indented by its pill. Whitespace inside a card, not more chrome. |
...
| Copyable values | *(Fluent: copy affordance)* | A short single-line value **is** the copy target (`CopyableValue`), not a glyph beside it: plain at rest, a `SurfaceMuted` pill on hover that hugs the value (left-aligned, equal side padding, capped at its container so a long value still clips), a `Copy` tooltip. The template leaves `Padding` to the TextBox itself (a `TemplateBinding` applied it twice), and the `-5` margin is the pill's border plus padding, so its text sits on the value's origin. `Tag` carries the clipboard payload, because the rendered string may be decorated (`TagText` and `OvModelSub` prefix `Tag `). One window-level `MouseLeftButtonUp` handler serves them all, since Lens rows are template-built; it copies only when `SelectionLength` is 0, so a drag still selects. Only a value that gets pasted into another tool earns it; the BitLocker key keeps a glyph, being the one that wraps. |
| Startup errors (`ErrorDialog`) | Modal Windows | A hand-themed WinForms window, since these fire before WPF and two of them fire *because* it failed. It draws the same chrome as `DialogWindow` (48px header, the X as a painted 10px glyph with the Destructive hover, 24px gutters) and sets its text in the embedded Montserrat through GDI+; the details box keeps the mono fallback. Popup chrome contract, and **no footer at all**: an error carries no decision, and a dismiss-only button is banned. One-line reason, a short action line in the docs' voice, then the exception behind a Details toggle on `TerminalBackground`, read-only so it can be pasted. Every call site passes reason and detail separately. |
...
| Tokens | *(visual language)* | Fonts: Montserrat (sans) and Geist Mono, embedded under `src/UI/Styles/Fonts` (OFL); Segoe UI and Cascadia Mono fall back. The docs site keeps Geist for its own text. One type ladder, 11 / 12 / 13 / 14 / 16 / 18, as roles in `Tokens.xaml`: `TextPaneTitle` 18/Bold > `TextTitle` 16 > `TextSubtitle` 14 > `TextBody` 13 > `TextMeta` 12 > `TextCaption` 11. Stat tiles use `StatValue` (18) + `MicroLabel` (11), buttons 13 Medium. A `TextBlock` takes a role, not an inline `FontFamily`/`FontSize`/`Foreground` triple, unless a trigger restyles it (a triggered `Style` cannot `BasedOn` a `DynamicResource`). A `TextBox` value stays inline, on the ladder. Badges are the 3-colour `Badge*` family, never hand-rolled. Radii: 10 cards, 8.4 controls/chips, 7 dropdown rows, 6 small chips. Glyphs pin `FontSymbol`. |
