...
| Truncatable text (hostnames, paths, names, versions) | **Progressive Disclosure** (hover reveals the rest) | Single-line variable-length text either trims with `TextTrimming="CharacterEllipsis"` (TextBlocks) or clips (the `SelectableText` TextBoxes — WPF TextBoxes have no trimming), and in both cases carries a `ToolTip` bound to the same value so the full text is one hover away. Copy-valuable values (hostname, service tag, person fields) stay `SelectableText` — selectability is why they are TextBoxes at all. **A same-value tooltip is gated on actually being cut:** on a `TextBlock`, bind it through a `Trigger` on that element's own `IsTextTrimmed` rather than setting `ToolTip` inline, so a value that fits shows nothing. Restating fully-visible text is the anti-pattern — it trains people to ignore tooltips, which costs you the ones that carry real detail (the identity pill's verdict, a folder row's full path). **`SelectableText` TextBoxes are the documented exception:** they clip instead of trimming and expose no `IsTextTrimmed`, so their tooltips stay unconditional; do not "fix" them. A tooltip that says something *different* from the visible text (`Label` -> `Path`, `UNVERIFIED` -> the full sentence) is never gated. Standing tooltips on labelled buttons are a few words at most, and a button whose own label already says it (`Run`) carries none. |
