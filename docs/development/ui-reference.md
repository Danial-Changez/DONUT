...
| Lens device card (model + service tag) | *(no ui-patterns.com entry - the catalogue's nearest matches govern data entry, not display)* | Each device row carries four facts past its name (OS, last logon, model, service tag), and they used to render as two visually identical dim mono lines, the second a run-on `Model - SN Serial`. The catalogue has nothing for this: **Structured Format** is about *entering* data to a fixed shape, and **Chunking** explicitly excludes information that must be "searched, scanned, or analyzed", which is exactly the pick-the-right-device task. So the call falls to tier 2, the app's own recipe. Model and tag take the **person-fields pairing** (MicroLabel over a `TitleTextPrimary` value) in a two-column grid mirroring the EMAIL/MANAGER, SAM/OFFICE block directly above them in the same pane, which also spends the card's width instead of stacking every fact in a narrow left ribbon. The label is **TAG**, matching the MACHINE stat tile's `Tag <service tag>` sub-line rather than inventing a second name for one fact (the old card said `SN`). The tag value stays `FontMono` (it gets transcribed off a sticker, so 0/O and 1/I must stay apart) while the model is `FontSans`, a product name. Per Fluent's proximity rule each pair keeps the app's 2px label/value rhythm and the block takes a 10px break from the OS line, so identity/context and physical identifiers read as two chunks rather than four undifferentiated lines. Each cell collapses on its own when SCCM hardware inventory is unreadable, `HasHardware` drops the whole row when neither field arrived, and the row's buttons moved to `VerticalAlignment="Top"` so `Add` stays level with the device name on the taller card. |
