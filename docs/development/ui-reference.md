...
| Machine-list status filter | **Module Tabs** | A `MicroLabel` "MACHINES" titles the pane (every other section carries one, and "machines" is the app's own user-facing word — cf. the "No machines yet" Blank Slate), with the segmented status chips below it and the `Clear` action on the right. `Clear` uses the neutral `ButtonSecondary` (not a ghost/tint) so it matches the other subordinate utility buttons — see *Colour hierarchy*. Module Tabs itself is silent on titles, so the title is a consistency call, not a pattern requirement. |
