...
| Truncatable text (hostnames, paths, names, versions) | **Progressive Disclosure** (hover reveals the rest) | Single-line variable-length text either trims with `TextTrimming="CharacterEllipsis"` (TextBlocks) or clips (the `SelectableText` TextBoxes — WPF TextBoxes have no trimming), and in both cases carries a `ToolTip` bound to the same value so the full text is one hover away. Copy-valuable values (hostname, service tag, person fields) stay `SelectableText` — selectability is why they are TextBoxes at all. **Same-value tooltips stay unconditional, because WPF cannot gate them.** `IsTextTrimmed` is a **UWP/WinUI** property — `System.Windows.Controls.TextBlock` has `TextTrimming` (how to trim) but nothing that reports *whether* it trimmed. A `Trigger` on it does not fail at parse: it breaks the whole enclosing template with `Set property 'System.Windows.FrameworkTemplate.Template' threw an exception`, so the pane using it will not load at all. Gating one needs an attached property that compares desired to rendered width, which is C# in `Donut.Mvvm`, not markup. Until that exists, a tooltip repeating fully-visible text is accepted noise. What *is* controllable: a tooltip that says something **different** from the visible text (`Label` -> `Path`, `UNVERIFIED` -> the full sentence) always earns its place; standing tooltips on labelled buttons are a few words at most; and a button whose own label already says it (`Run`) carries none. |
