...
4. When no updates are found, the apply is skipped automatically.

The updates list is also copied to the clipboard for pasting into tickets.

## Reboots

- DONUT parses the DCU log for reboot-required vs auto-reboot outcomes and shows a
  completion popup listing machines that need a **manual reboot**.
