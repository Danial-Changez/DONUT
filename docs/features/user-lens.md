...
- **Their software** — the deployments targeted at the user: install-intent
  ...
  **Software** button swaps the device list for it and back; its lookup runs in
  parallel with the person lookup, so neither waits on the other. A site can narrow
  the list with the optional `lensSoftwareCollectionFilter` config regex on
  collection names (blank, the default, shows everything).
