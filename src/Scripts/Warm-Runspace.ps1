...
.NOTES
    ...
    (localhost CIM / task / assembly loads); it must never depend on a reachable
    DC / SCCM / AD.
#>
...
# The binary CIM + ScheduledTasks stacks (Get-CimInstance / *-ScheduledTask, hit by the
# workers and PersonLensService.EnsureAgent) take the process-wide CLR loader lock on
# their FIRST call - importing alone doesn't pay that cost - so invoke each cheap, local
# cmdlet here on the warm barrier instead of on a pool job during startup.
try {
