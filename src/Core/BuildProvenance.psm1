...
.DESCRIPTION
    ...
    the install is a clone, plus the pwsh/CLR versions, machine name and OS build.
    Logged once right after "DONUT starting up." (see DonutApp.ps1).
...
.NOTES
    ...
    On an MSI install the SHA probe fails and the stamp falls through to "unknown",
    so a prod log identifies its build by the MSI's registry DisplayVersion instead.
#>
class BuildProvenance {
    ...
    # Build identity: git SHA (+dirty) on clones, else unknown.
